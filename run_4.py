...
import logging
import os
from config_private import CONFIG_DATASET, CONFIG_SHOPPING_4, CONFIG_CLASSIFIEDS, CONFIG_CLASSIFIEDS_RESET_TOKEN, \
    CONFIG_REDDIT, CONFIG_WIKIPEDIA, CONFIG_HOMEPAGE, OPENAI_API_KEY, OPENAI_URL

os.environ["DATASET"] = CONFIG_DATASET
os.environ["SHOPPING"] = CONFIG_SHOPPING_4
...
def config() -> argparse.Namespace:
    ...
    parser.add_argument("--test_config_base_dir", type=str, default="config_files/vwa/test_shopping4")
    ...
    parser.add_argument("--test_start_idx", type=int, default=75)
    ...
    for config_file in config_file_list:
        ...
        try:
            ...
            while True:
                ...
                print("current depth: {}".format((len(trajectory) - 1) / 2))
                early_stop_flag, stop_info = early_stop(
                    trajectory, max_steps, early_stop_thresholds
                )

                print(f"开始初始节点扩展")
                start_init_expand = time.time()
                if early_stop_flag:
                ...
                time_init_expand = time.time() - start_init_expand
                print(f"初始节点扩展: {time_init_expand :.4f}秒")
                ...
                if args.agent_type == "search" and type(action) == list:
                    ...
                    def take_action_and_score(a, curr_trajectory, curr_action_history,
                        ...
                        try:
                            ...
                            score = 0
                        time_value = time.time() - start_value
                        print(f"价值函数: {time_value:.4f}秒")
                        next_actions = []
                        ...
                        time_expand = time.time() - start_expand
                        print(f"节点扩展: {time_expand:.4f}秒")
            ...
            logger.info(f"[Value Model] {args.value_function}")
